...
const i_env = {
   debug: !!process.env.SGREP_DEBUG,
   server: {
      host: process.env.SGREP_HOST || '127.0.0.1',
      port: parseInt(process.env.SGREP_PORT || '8080'),
      staticDir: process.env.SGREP_STATIC_DIR?i_path.resolve(process.env.SGREP_STATIC_DIR):null,
      httpsCADir: process.env.SGREP_HTTPS_CA_DIR?i_path.resolve(process.env.SGREP_HTTPS_CA_DIR):null,
   },
};
...
server.listen(i_env.server.port, i_env.server.host, () => {
   console.log(`SGREP SERVER is listening at ${i_env.server.host}:${i_env.server.port}`);
});
